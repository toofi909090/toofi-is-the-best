...
function sendPatch(channel: Channel, body: Record<string, any>, bypass = false) {
    ...
    Object.keys(usersVoice).forEach(key => {
        ...
        const user = UserStore.getUser(userVoice.userId);

        // تجاهل بوتات الأغاني + أي نوع بوت ثاني
        if (user?.bot) return;

        // تجاهل نفسي
        if (!bypass && userVoice.userId === myId) return;
...
export default definePlugin({
    ...
    description:
        "This plugin allows you to perform multiple actions on an entire channel (move, mute, disconnect, etc.) but now ignores music bots.",
    authors: [Devs.D3SOX],
